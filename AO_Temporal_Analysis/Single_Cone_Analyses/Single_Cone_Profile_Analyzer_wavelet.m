...
%     controlMedian(:,i) = ControlMedianResp;    
    ...
    single_cone_response(:,i) = MedianResp;%+abs(MedianResp);


    if logmode        
        single_cone_response(:,i) = log(single_cone_response(:,i));
    end
end

valid = true(size(allcoords,1),1);
...
close all;
lessdensitometry_threshold = (densitometry_fit_amplitude<=DENSTOMETRY_THRESHOLD);% & (single_cone_response(:,end)<RESPONSE_THRESHOLD) ;

% lessdensitometry_threshold = (single_cone_response(:,end)<RESPONSE_THRESHOLD) 
...
%     plot([-10 10],[-10 10],'k');
    ...
    generate_spatial_map(single_cone_response(:,i), allcoords, valid_densitometry,[], single_cone_mat_files(loadinds(i)), '_VS', saveplots, lessdensitometry_threshold);
...
response_of_interest = single_cone_response(valid,end);
